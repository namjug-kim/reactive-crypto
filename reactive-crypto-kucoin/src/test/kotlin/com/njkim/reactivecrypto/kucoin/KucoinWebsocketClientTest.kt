...
class KucoinWebsocketClientTest {
    ...
    fun `tick data subscribe`() {
        ...
        val tickDataFlux = KucoinWebsocketClient()
        ...
        StepVerifier.create(tickDataFlux.limitRequest(3))
            ...
            .assertNext {
                ...
                Assertions.assertThat(it.exchangeVendor)
                    .isEqualTo(ExchangeVendor.KUCOIN)
    ...
    fun `orderBook subscribe`() {
        // given
        val targetCurrencyPair = CurrencyPair.parse("BTC", "USDT")
        val orderBookFlux = KucoinWebsocketClient()
            .createDepthSnapshot(listOf(targetCurrencyPair))
            .doOnNext { log.debug { it } }

        // when
        StepVerifier.create(orderBookFlux.limitRequest(30))
            .expectNextCount(28)
            // then
            .assertNext {
                Assertions.assertThat(it).isNotNull
                ...
                Assertions.assertThat(it.asks[0].price)
                    ...
                    .isGreaterThanOrEqualTo(it.bids[0].price)
